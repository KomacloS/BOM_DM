...
def _dedupe_part_test_map(bind) -> None:
    import sqlalchemy as sa
    from sqlalchemy import text

    insp = sa.inspect(bind)
    if "part_test_map" not in insp.get_table_names():
        return

    dup_sql = text(
        """
        SELECT part_id, profile, COUNT(*) AS c
        FROM part_test_map
        GROUP BY part_id, profile
        HAVING COUNT(*) > 1
        """
    )

    rows = bind.execute(dup_sql).fetchall()
    if not rows:
        return

    for part_id, profile, _count in rows:
        cand_sql = text(
            """
            SELECT part_id, test_id, profile
            FROM part_test_map
            WHERE part_id = :pid AND profile = :prof
            ORDER BY test_id DESC
            """
        )
        candidates = bind.execute(cand_sql, {"pid": part_id, "prof": profile}).fetchall()
        if not candidates:
            continue
        ...
        for row in to_delete:
            ...
            bind.execute(
                text(
                    """
                    DELETE FROM part_test_map
                    WHERE part_id = :pid AND test_id = :tid AND profile = :prof
                    """
                ),
                {"pid": p_id, "tid": t_id, "prof": prof},
            )
